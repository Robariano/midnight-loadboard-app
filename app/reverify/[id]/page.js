...
export default function Reverify({ params }) {
  ...
  async function handleSubmit(e) {
        ...
        const res = await fetch(`/api/carriers/${carrierId}/reverify`, {
                ...
                body: JSON.stringify({ carrier_secret: secret, ...form }),
        });
  ...
  if (status === "success") {
        return (
          ...
          Your updated documents were submitted for review. We review everything within 24-48 hours,
                      and you will be able to claim loads again once approved.
            </p>
            </div>
    );
}
  ...
  return (
      ...
      <p style={{ color: "#888", marginBottom: 24 }}>
        Use this if your insurance has expired or your documents are out of date. Enter the carrier
        secret you saved at signup, then submit updated links. We review everything within 24-48
        hours.
          </p>
      <form onSubmit={handleSubmit}>
                  <label style={labelStyle}>Carrier ID</label>
        <input style={{ ...inputStyle, opacity: 0.6 }} value={carrierId} disabled readOnly />

                  <label style={labelStyle}>Your Carrier Secret</label>
        <input
          style={inputStyle}
          type="password"
          required
