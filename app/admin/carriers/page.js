...
export default function AdminCarriers() {
  ...
  if (error === "unauthorized") {
        return (
          ...
          <a href="/admin" style={{ color: "#5c5cff" }}>Go to admin login &rarr;</a>
  </div>
    );
}
  ...
  return (
      ...
      <p style={{ color: "#888", marginBottom: 20 }}>
        <a href="/admin/flags" style={{ color: "#5c5cff" }}>View coverage flags &rarr;</a>
          </p>
{carriers.map((c) => (
          <div key={c.id} style={{
...
{" "}<span style={{ color: "#666" }}>(only needed if they signed up before this was added - send it to them once, privately)</span>
          ...
          <p style={{ fontSize: 12, color: "#888", margin: "0 0 4px" }}>
            Reverify link: <code style={{ color: "#5c5cff" }}>https://midnightloadboard.com/reverify/{c.id}</code>
            {" "}<span style={{ color: "#666" }}>(send this if they need to resubmit documents - they'll still need their secret)</span>
              </p>
