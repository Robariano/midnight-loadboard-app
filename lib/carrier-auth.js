...
function sign(value) {
      // A falsy/empty secret must never silently produce a "validly signed"
  // cookie - that previously let carrier sessions get issued that could
  // never actually verify (empty string is falsy, so the check in
  // getCarrierIdFromRequest rejected every one of them). Fail loudly here
  // instead so a misconfigured env var breaks signup/login immediately and
  // visibly, rather than breaking session verification silently.
  if (!SESSION_SECRET) {
          throw new Error("[carrier-auth] CARRIER_SESSION_SECRET is not set - cannot sign carrier sessions.");
  }
...
export function createSessionCookie(carrierId) {
      ...
      return `carrier_session=${encodeURIComponent(token)}; HttpOnly; Secure; Path=/; SameSite=Lax; Max-Age=${SESSION_MAX_AGE_MS / 1000}`;
}
export function clearSessionCookie() {
      return `carrier_session=; HttpOnly; Secure; Path=/; SameSite=Lax; Max-Age=0`;
}
